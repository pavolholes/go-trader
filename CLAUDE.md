...
- Notifications: `MultiNotifier`; paper routes via `resolveChannelKey`. `SendToScopeChannels` resolves paper destinations from the paper roster (`resolveTradeChannel`, rebuilt on `ReloadConfig`), never a `-paper` suffix scan; falls back to `SendToAllChannels` only when that set is empty.
