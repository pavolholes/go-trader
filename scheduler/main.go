				...
				case "perps":
					...
					if sc.Platform == "okx" {
						...
						if result, signalStr, price, ok := runBloFinCheck(sc, prices, blofinPosCtx, cfg.Regime, notifier, logger); ok {
							...
							hurstDecision := advanceHurstGate(sc, storeRegime, cfg.Regime, stratState, &mu, blofinPosQty)
							if hurstDecision.Holds && pausedBlocksSignal(result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)) {
								logger.Info("Hurst gate: %s signal suppressed - %s (#1411)", signalStr, hurstDecision.Detail)
								result.Signal = 0
							}
							if sc.Paused && pausedBlocksSignal(result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)) {
								logger.Info("Paused: %s signal suppressed - position-increasing actions held while paused (#1150)", signalStr)
								result.Signal = 0
							}
							if sr.DailyLossEntriesHeld && pausedBlocksSignal(result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)) {
								logger.Info("Daily loss limit: %s signal suppressed - entries held until UTC rollover (#1269)", signalStr)
								result.Signal = 0
							}
							if sr.NotionalBlocked && pausedBlocksSignal(result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)) {
								logger.Warn("Notional cap: %s signal suppressed - new opens blocked, exits continue (#1344)", signalStr)
								result.Signal = 0
							}
							if persistenceHold && pausedBlocksSignal(result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)) {
								logger.Warn("Persistence hold: %s signal suppressed", signalStr)
								result.Signal = 0
							}
							if capBlocked, capWhy := exposureCapBlocksSignal(sr.ExposureCapStatus, extractAsset(sc), result.Signal, result.CloseFraction, blofinPosQty, blofinPosSide, PerpsAllowsLong(sc), PerpsAllowsShort(sc)); capBlocked {
								logger.Warn("Exposure cap: %s signal suppressed - %s (#1270)", signalStr, capWhy)
								result.Signal = 0
							}
							mu.Lock()
							...
							if entry, applied, legacyFallback := applyRegimeDirectionalPolicy(&sc, currentDirRegime, posDirRegime, blofinPosQty, dirCertStates); applied {
								...
								logger.Info("Regime directional policy: regime=%s -> direction=%q invert_signal=%t", regimeKey, entry.Direction, entry.InvertSignal)
								if legacyFallback {
									if _, loaded := regimeDirectionalLegacyWarned.LoadOrStore(sc.ID, struct{}{}); !loaded {
										logger.Warn("Regime directional policy: legacy position without stamped regime; resolving against current regime=%q.", regimeKey)
									}
								}
							}
